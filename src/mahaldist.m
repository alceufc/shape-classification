...
%   but the former is computed more efficiently. In this case, D(i) is the
%   Mahalanobis distance from the point X(i,:) to the centroid of set Y (i.e.,
%   the mean of the rows in Y) with respect to the variance in the set Y (i.e.,
%   the variance matrix of the rows in Y).
%
%   MAHALDIST(X) is the same as MAHALDIST(X, X), but the former is computed
%   more efficiently.
%
%   NOTE: The Mahalanobis distance is sometimes defined as the square root of
%   the values returned by MAHALDIST.
%
...
%   the arguments are swapped compared to earlier versions.
%
...
%   Time-stamp:  2008-03-25 18:52:04 +01:00
